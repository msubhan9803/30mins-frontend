...
export default function AddOrganizationServices({userStripeAccount}) {
  const {t} = useTranslation('common');
  const {t: tpage} = useTranslation('page');

  const router = useRouter();
  const {mode, stepType, sid} = router.query;

  const stype = 'organization';

  const crumbs = [
    {title: tpage('Home'), href: '/'},
    {
      title:
        mode === 'create' ? tpage('Add Organization Service') : tpage('Edit Organization Service'),
      href:
        mode === 'create'
          ? '/user/organization-services/add-organization-service/?mode=create&stepType=organization'
          : `/user/organization-services/add-organization-service/?mode=edit&sid=${sid}`,
    },
  ];

  const {data: session} = useSession();
  const {user, hasCalendar, hasOrgExtention, hasOrgServiceCats, hasOrgs} = useContext(UserContext);
  ...
  const submitService = async () => {
    ...
    const response = await createOrgService({
      variables: {
    ...
    try {
      ...
      if (action === 'next') {
        ...
        if (!update) {
          ...
          const stageData = values && stageLocalData(values, stype);
          const temp = {...stageData} as any;
          delete temp.dueDate;

          const response = await editOrgService({
            variables: {
              documentId: sid,
              organizationServiceData: temp,
              token: session?.accessToken,
            },
          });
    ...
    return (
      <PostLoginLayout>
        <Header
          ...
          heading={
            mode === 'create'
              ? tpage('page:Add Organization Service')
              : tpage('page:Edit Organization Service')
          }
        />
        <SecLoader />
      </PostLoginLayout>
        ...
        <div className={classNames(['relative border-gray-200 duration-800'])}>
          ...
          <Transition step={step} show={created}>
            <div className='flex flex-col items-center justify-center'>
              ...
              <h3 className='text-xl font-medium mt-4 text-gray-600'>{t('service_added_title')}</h3>
              ...
              <div className='flex gap-4 mt-4'>
                ...
                <Button
                  onClick={() =>
                    router.push('/user/organization-services/all-organization-services')
                  }
                  variant='outline'
                >
                  {t('go_to_services')}
