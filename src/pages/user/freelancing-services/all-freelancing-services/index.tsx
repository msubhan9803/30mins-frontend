...
export default function AllFreelancingServices() {
  ...
  const crumbs = [
    ...
    {title: tpage('Freelancing Services'), href: '#'},
    {
      title: tpage('All Freelancing Services'),
  ...
  const addServicePress = () => {
    if (!hasCalendar) {
      ...
      router.push('/user/freelancing-services/add-services/?mode=create&stepType=FREELANCING_WORK');
    }
  };
  ...
  return (
    <PostLoginLayout>
      <Head>
        <title>{tpage('All Freelancing Services')}</title>
      </Head>
      <Header crumbs={crumbs} heading={tpage('All Freelancing Services')} />
      <div className='pb-4'>
        ...
        {serviceData?.filter &&
        serviceData?.filter(el => el.serviceType === SERVICE_TYPES.FREELANCING_WORK)?.length > 0 ? (
          <>
                ...
                <tbody>
                  {serviceData.length > 0 &&
                    ...
                    serviceData.map((service, index) => {
                      if (service.serviceType === SERVICE_TYPES.FREELANCING_WORK)
                        return (
          ...
          <Error
            ...
            title={t('common:freelance_no_x_added_main')}
            description={t('common:freelance_no_x_added_description')}
            linkText={t('common:freelance_learn_more')}
            linkURL='/blog/help_user_freelancing_services_all_freelancing_services/'
          />
        )}
      </div>
