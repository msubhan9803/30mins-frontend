...
const AddService = ({userStripeAccount, hasPaidMeetingExtension}) => {
  //
  ...
  const crumbs = [
    ...
    {
      title: tpage(mode === 'create' ? 'Add Service' : 'Edit Service'),
      href:
        mode === 'create'
          ? '/user/meeting-services/add-services/?mode=create&stepType=MEETING'
          : `/user/meeting-services/add-services/?mode=edit&sid=${sid}`,
    },
  ];
    ...
    return (
      <PostLoginLayout>
        <Header
          crumbs={crumbs}
          heading={`${
            mode === 'create' ? t('add_new_service_type') : t('edit_service_type')
          } ${serviceType}`}
        />
        <SecLoader />
      </PostLoginLayout>
          ...
          <>
            ...
            <Transition step={step} show={stepName === 'ServicePayment'}>
              <ServicePayment
                ...
                step={step}
                mode={mode}
                hasPaidMeetingExtension={hasPaidMeetingExtension}
              />
            </Transition>
            ...
            <Transition step={step} show={created}>
              <div className='flex flex-col items-center justify-center'>
                ...
                <div className='flex gap-4 mt-4'>
                  ...
                  <Button
                    onClick={() => router.push('/user/meeting-services/all-meeting-services')}
                    variant='outline'
                  >
                    {t('go_to_services')}
                  </Button>
...
export const getServerSideProps: GetServerSideProps = async context => {
  try {
    ...
    const hasPaidMeetingExtension = userData?.accountDetails?.activeExtensions.includes(
      stripeProductIDs.EXTENSIONS.PAID_MEETINGS
    );
