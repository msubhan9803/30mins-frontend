...
const Confirm = ({meeting, user, serviceExists}) => {
  ...
  const confirmMeeting = async (meetingData, values) => {
    if (reason === '') {
    ...
    if (res?.data?.success) {
      setLoading(false);
      showNotification(NOTIFICATION_TYPES.success, 'Meeting Confirmed successfully', false);
      // eslint-disable-next-line no-restricted-globals
      router.replace(location.href.replace('confirm', '').toString());
    }
  };
  const onCancel = () => {
            ...
            <>
              ...
              <div className='px-4 mt-5 sm:mb-4 text-right sm:px-6'>
                ...
                <Button
                  ...
                  type='submit'
                  onClick={() => {
                    confirmMeeting(meetingDetails, reason);
                  }}
