...
const Cancel = ({meeting, serviceExists}) => {
  ...
  const cancelMeeting = async (meetingData, values) => {
    try {
      if (reason === '') {
      ...
      if (res?.data?.success) {
        showNotification(NOTIFICATION_TYPES.success, 'Meeting cancelled successfully', false);
        setLoading(false);
        // eslint-disable-next-line no-restricted-globals
        router.replace(location.href.replace('cancel', '').toString());
      }
    } catch (error) {
      setLoading(false);
            ...
            <>
              ...
              <div className='px-4 mt-5 sm:mb-4 text-right sm:px-6'>
                ...
                <Button
                  ...
                  variant='solid'
                  onClick={() => {
                    cancelMeeting(meetingDetails, reason);
                  }}
                  disabled={loading || reason === '' || cancelLoading}
