...
import RoundRobin from 'components/PostLogin/RoundRobin';
...
const OrganizationPage = ({user}) => {
  ...
  const crumbs = [
    ...
    {title: t('page:Round Robin Meetings'), href: '/round-robin'},
  ];

  return (
    <PostLoginLayout>
      <Header crumbs={crumbs} heading={t('page:Round Robin Meetings')} />
      <Head>
        <title> {t('page:Round Robin Meetings')}</title>
      </Head>
      <RoundRobin user={user} />
    </PostLoginLayout>
  );
};
