    ...
    >
      ...
      <div className='box col-start-2 col-span-2 w-full flex flex-col overflow-y-auto'>
        ...
        <p className='line-clamp-3 text-sm text-gray-500 font-bold'>
          {t('common:by')} {item?.userId?.personalDetails?.name},{' '}
          {item?.userId?.personalDetails?.headline}
        </p>
      ...
      <div className='box px-2 py-2 col-start-3 text-mainBlue flex items-end justify-end'>
        <Link
          href={
            item.serviceType === SERVICE_TYPES.ROUND_ROBIN
              ? `/org/${item?.organizationId?.slug}/${item?.slug}`
              : `/${item?.userId.accountDetails?.username}/${item?.slug}`
          }
          passHref
        >
          {t('common:book_now')}
