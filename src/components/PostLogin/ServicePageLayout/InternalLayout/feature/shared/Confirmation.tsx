...
const Confirmation = ({values, paymentInvoice, hidden}: IProps) => {
  const {t} = useTranslation();
  const {data: session} = useSession();
  const {selectedTime, serviceData, user, selectedBookerTimezone} = values;
  dayjs.tz.setDefault(selectedBookerTimezone);
  useEffect(() => {}, [session?.accessToken]);
  ...
  const eventLink = () => {
    ...
    const event = ics.createEvent({
      start,
      startInputType: 'utc',
      title: `${serviceData.title} with ${user?.accountDetails.username}`,
              ...
              <div className='mt-3 text-center sm:mt-5'>
                ...
                <div className='mt-2'>
                  <p className='text-sm text-gray-500'>
                    {t('meeting:you_sche_desc')} {values.user?.accountDetails?.username}.
                  </p>
                </div>
                ...
                <Link
                  href={encodeURI(
                    `https://calendar.google.com/calendar/render?action=TEMPLATE&dates=${dayjs(
                      ...
                      .format('YYYYMMDDThhmmss')}&text=${serviceData.title} with ${
