...
function SignInSection() {
  ...
  const otpSubmitHandler = async (values, {setFieldError}) => {
    try {
      ...
      if (res?.ok === true) {
        hideModal();
        await modalProps.onAuthComplete();
          ...
          <>
            <form className='flex flex-col gap-4' onSubmit={handleSubmit} noValidate>
              ...
              <Button
                ...
                onClick={() => setErors('')}
              >
                {loading ? t('common:sending_otp') : t('common:download_sign_in_otp')}
              </Button>
            </form>
          </>
        )}
        ...
        {({handleSubmit, errors: formErrors, setFieldValue}) => (
          <>
            ...
            <form className='flex flex-col gap-4' onSubmit={handleSubmit}>
              ...
              <Button className={'pt-6 pb-6'} variant='solid' type='submit' disabled={loading}>
...
function SignUpSection() {
  ...
  return !showOtpInput ? (
        ...
        {({handleSubmit, values, errors, handleChange, setFieldValue}) => (
          <>
            <form className='flex flex-col gap-4' onSubmit={handleSubmit}>
              ...
              <Button className={'pt-6 pb-6'} variant='solid' type='submit' disabled={loading}>
                {loading ? t('common:signing_up') : t('common:download_sign_up_otp')}
              </Button>
            </form>
          </>
        )}
    ...
    <div>
      <Formik initialValues={OTP_STATE} onSubmit={otpSubmitHandler} validationSchema={OTP_YUP}>
          ...
          <>
            ...
            <form className='flex flex-col gap-4' onSubmit={handleSubmit}>
              ...
              <Button
                ...
                variant='solid'
                type='submit'
                disabled={loadingVerifyOTP || loading}
              >
...
const FreeDownloadAuthModal = () => {
  const {t} = useTranslation();

  const [isSigninSection, setIsSigninSection] = useState(false);

  const toggleSection = () => setIsSigninSection(!isSigninSection);

  return (
    <Modal small modalWrapperClass='pb-20' title='Download' medium>
      <div className=''>
        <span className='text-center font-medium'>
          {t('common:downloading_ebook_requires_an_account')}&nbsp;
          <span
            className='font-medium text-mainBlue hover:text-mainBlue cursor-pointer'
            onClick={toggleSection}
          >
            {isSigninSection
              ? t('common:have_no_account_with_30mins_signup')
              : t('common:already_have_an_account_with_30mins_signin')}
          </span>
        </span>
        <p className='text-start text-base sm:text-base text-gray-900 pt-4'>
          {isSigninSection ? t('common:Signin_title') : null}
        </p>
...
export default FreeDownloadAuthModal;
