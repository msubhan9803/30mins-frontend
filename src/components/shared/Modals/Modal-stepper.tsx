...
const Modal = ({title, children, medium, small, extraSmall, max, step, size}: Props) => {
    ...
    <>
      ...
      >
        <div className='relative min-h-screen max-h-screen text-center flex justify-center items-center'>
          ...
          ></div>
          <div
            className={`inline-block bg-white rounded-lg mx-4 sm:mx-0 px-4  pb-4 text-left shadow-xl transform transition-all overflow-y-scroll md:overflow-auto lg:overflow-auto md:overflow-x-hidden lg:overflow-x-hidden align-middle max-w-4xl w-full p-6 sm:my-8 sm:align-middle sm:max-w-4xl max-h-[90vh]`}
            style={{
          ...
          >
            ...
            <Stepper max={max} step={step} size={size} />
            {children}
          </div>
        </div>
      </div>
