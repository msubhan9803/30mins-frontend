...
const Table = () => {
  ...
  const {
    data: attendeeEventsData,
    loading: clientLoading,
    refetch: refetchAttendeeEvents,
  } = useQuery(eventQueries.getEventsForAttendeeOrOrganizer, {
    variables: {
      token: session?.accessToken,
      searchParams: {
  ...
  const attendeeEvents = attendeeEventsData?.getEventsForAttendeeOrOrganizer?.eventsData;
  const attendeeEventsCount = attendeeEventsData?.getEventsForAttendeeOrOrganizer?.totalRecords;
  ...
  useEffect(() => {
    ...
    if (attendeeEvents) {
      setTableData(attendeeEvents);
      tempEventsList = attendeeEvents;

      dynamicPageNumbers(attendeeEventsCount, rowsPerPage);
    }
  }, [user, attendeeEvents, month, rowsPerPage]);
  ...
  const handleFilter = () => {
    if (filterText !== '') {
      const filterEvents = tempEventsList.filter(event =>
        event.serviceTitle.toLowerCase().includes(filterText.toLocaleLowerCase())
      );

      setTableData(filterEvents);
    } else {
      setTableData(tempEventsList);
    }
  };
  ...
  return (
    <>
      <div className='container mx-auto'>
        <div className='flex items-center justify-end md:justify-end lg:justify-end mb-5  md:mt-3 lg:mt-5 my-3'>
          <input
            type='text'
            id='filter'
            name='filter'
            ...
            placeholder={t('common:event_name')}
          />
          <button
            type='button'
              ...
              <div className=''>
                ...
                >
                  <thead className='border-b bg-gray-50'>
                    <tr>
                      ...
                      <th
                        ...
                        className='min-w-[20%] w-[20%] max-w-[20%] text-sm font-bold text-black px-2 py-2 whitespace-nowrap'
                      >
                        {t('common:attendee_limit')}
                      </th>
                      <th
                        scope='col'
                        className='min-w-[20%] w-[20%] max-w-[20%] text-sm font-bold text-black px-2 py-2 whitespace-nowrap'
                      >
                        {t('common:registered_at')}
                      </th>
                  ...
                  <tbody className='overflow-y-scroll'>
                    {tableData &&
                      tableData.map(el => {
                        const username = el.userId?.accountDetails?.username ?? '';

                        return (
                          <tr key={el._id} className='bg-white border-b w-20 py-2'>
...
function ShowActionMenus({username, slug, id, refetch}) {
  const {t} = useTranslation('common');

  const {data: session} = useSession();
  const [cancelAttendeeMutation, {loading}] = useMutation(mutations.cancelAttendeeEvent);

  const [isOpen, setIsOpen] = useState(false);

  const handleCancelAttendee = async () => {
    try {
      await cancelAttendeeMutation({
        variables: {
          token: session?.accessToken,
          eventId: id,
        },
      ...
      toast.success(t('toast_event_attendee_cancel'));
  ...
  return (
    <>
      <div className='flex items-center justify-center gap-2'>
        <a href={`/${username}/events/${slug}`} target='_blank' rel='noreferrer'>
          <ArrowTopRightOnSquareIcon title='View Event' className='w-6 h-6 cursor-pointer' />
        </a>
      ...
      <ConfirmDialog
        title={t('txt_leave_event_title')}
        description={t('txt_leave_event_desc')}
        isOpen={isOpen}
        setIsOpen={setIsOpen}
      >
        ...
        <Button variant='solid' onClick={handleCancelAttendee}>
          {loading ? <Loader /> : t('btn_leave')}
        </Button>
      </ConfirmDialog>
    </>
  );
