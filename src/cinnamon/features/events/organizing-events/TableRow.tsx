...
const TableRow = ({service, refetch, withoutType}: {service; refetch; withoutType?: boolean}) => {
  ...
  const downloadQR = () => {
    const id = `downloadqrcode${service.serviceSlug}`;
    const canvas = document.getElementById(id) as HTMLCanvasElement;
    if (canvas !== null) {
      const pngUrl = canvas.toDataURL('image/png').replace('image/png', 'image/octet-stream');
      const downloadLink = document.createElement('a');
      downloadLink.href = pngUrl;
      downloadLink.download = `download_${service.serviceSlug}.png`;
  ...
  const EventImage = service?.serviceImage
  ...
  return (
    <Row styles='text-sm'>
      <Cell styles='w-24 pr-0 hidden md:table-cell'>
        <div className='w-24 h-24 overflow-hidden flex items-center rounded-lg bg-gray-100'>
      ...
      <Cell styles='max-w-[320px] lg:max-w-[400px]'>
        <div className='flex flex-col'>
          <div>
            ...
            <div className='flex items-center space-x-2 mt-1'>
              <button
                onClick={() => {
                  ...
                  navigator.clipboard.writeText(
                    ...
                    `${location?.origin}/${user?.username}/events/${service.serviceSlug}`
                  );
                }}
              ...
              <p className='truncate w-5/6 text-mainBlue border-opacity-30 text-[15px]'>
                <a
                  // eslint-disable-next-line no-restricted-globals
                  href={`${location?.origin}/${user?.username}/events/${service.serviceSlug}`}
                  className='text-mainBlue'
                  target='_blank'
                  rel='noreferrer'
                >
                  https://30mins.com/{user?.username}/events/{service.serviceSlug}
                </a>
              </p>
            </div>
          </div>
          <div>
            <div className='flex items-start justify-start'>
              <QRCode
                id={`displayqrcode${service.serviceSlug}`}
                value={`https://30mins.com/${user?.username}/events/${service.serviceSlug}`}
                size={100}
                level={'H'}
                includeMargin={true}
              />
            ...
            <div className='hidden'>
              <QRCode
                id={`downloadqrcode${service.serviceSlug}`}
                value={`https://30mins.com/${user?.username}/events/${service.serviceSlug}`}
                size={2000}
                level={'H'}
                includeMargin={true}
        ...
        <div className='flex lg:hidden mt-2 space-x-2 justify-between'>
          ...
          <Actions
            ...
            type={service.serviceRecurring}
          />
        </div>
      </Cell>
      {!withoutType && <Cell styles='text-sm hidden lg:table-cell'>{t(service.serviceType)}</Cell>}
      <Cell styles='text-sm hidden lg:table-cell font-semibold'>{service.eventStatus}</Cell>
      <Cell styles='text-sm hidden lg:table-cell'>
        {!service.isPublic ? t(`Private`) : t('Public')}
      </Cell>
      <Cell styles='text-sm hidden lg:table-cell'>
        <Actions
          button={
