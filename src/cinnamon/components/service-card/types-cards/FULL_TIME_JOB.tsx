...
export default function FULL_TIME_JOB({service, username}: IPropsCard) {
  ...
  return (
    ...
    >
      <div className='row-start-1 col-span-2 row-span-1'>
        <img
          src={service?.image ? service?.image : defaulteImage.FULL_TIME_JOB}
          alt='avatar'
      ...
      <div className='col-span-3 col-start-1 place-items-center flex justify-start items-end gap-1'>
        <span className='font-bold text-sm md:text-mds '>{t('common:available_for')}</span>
        <span className='font-bold text-sm md:text-mds text-mainBlue'>
          {' '}
          Full {t('common:time_job')}
        </span>
      </div>
